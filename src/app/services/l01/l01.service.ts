...
// Importar L01Catalog desde el servicio de catálogos
...
export interface L01Record {
  ...
  codigoTipoIdentificacion: number;  // ID de Tabla 4
  codigoEmisor: number;              // ID de Tabla 164
  codigoClasificacionEmisor: number; // ID de Tabla 173
  codigoTipoEmisor: number;          // ID de Tabla 73
...
// Nueva interfaz normalizada como L02
export interface L01Resume {
  tipoIdentificacion: L01Catalog;    // Objeto con {id, codigo, descripcion}
  identificacion: string;            // RUC o código extranjero directo
  clasificacion: L01Catalog;         // Objeto con {id, codigo, descripcion}
  tipoEmisor: L01Catalog;            // Objeto con {id, codigo, descripcion}
}
...
export class L01Service {
  ...
  /**
   * Obtener todos los registros L01 (método original)
   */
  getAll(): Observable<L01Record[]> {
    const url = `${this.baseUrl}/structures/l01/resume`;
  ...
  /**
   * Obtener resumen L01 normalizado como L02
   */
  getResume(): Observable<L01Resume[]> {
    const url = `${this.baseUrl}/structures/l01/resume`;
    return this.http.get<L01Resume[]>(url).pipe(
      catchError(this.handleError.bind(this))
    );
  }
